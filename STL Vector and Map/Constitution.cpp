...
const string INPUT_FILE_NAME = "/Users/indirabobburi/workspace/STL Vector and Map/STL Vector and Map/USConstitution.txt";
...
void time_word_insertions(ifstream& in, WordVector& v, WordMap& m)
{
    ...
    while (getline(in, line))
    {
        ...
        while (i < length)
        {
            found = line.find_first_not_of(alphabet, i);
            if(found != std::string::npos)
            {
                text = line.substr(i, found-i);
                i = line.find_first_of(alphabet, found);
                if(i == -1) i = length;
            }
            else
            {
